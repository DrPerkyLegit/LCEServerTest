...
#include "../Minecraft.Client/CommandDispatcher.h"
...
static bool ExecuteConsoleCommand(MinecraftServer *server, const wstring &rawCommand)
{
	...
	return CommandDispatcher::OnConsoleCommand(rawCommand);
}
...
#endif
	Logger::Info((L"Starting Minecraft Server | Protocol(" + std::to_wstring(MINECRAFT_NET_VERSION) + L") - Build(" + VER_PRODUCTVERSION_STR_W + L")").c_str());
...
//					PIXReportCounter(L"Server ticks",(float)tickcount);
			...
			for(int i=0;i<XUSER_MAX_COUNT;i++)
			{
				...
				case eXuiServerAction_SaveGame:
					...
					break;
				case eXuiServerAction_StopServer:
					HaltServer();
					break;
